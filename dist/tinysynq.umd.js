(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('tslog'), require('dotenv/config'), require('better-sqlite3'), require('nanoid'), require('uWebSockets.js'), require('worker_threads')) :
  typeof define === 'function' && define.amd ? define(['tslog', 'dotenv/config', 'better-sqlite3', 'nanoid', 'uWebSockets.js', 'worker_threads'], factory) :
  (global = global || self, global.tinysynqNode = factory(global.tslog, null, global.betterSqlite3, global.nanoid, global.uWS, global.workerThreads));
})(this, (function (tslog, config, DB, nanoid, uWS, worker_threads) {
  ...
  const startTinySynqServer = ts => {
    ...
    server = app.listen(port, token => {
      if (token) {
        console.log('TinySynq server listening on port', port, 'from thread', worker_threads.threadId);
      } else {
        console.log('Failed to listen on port', port, 'from thread', worker_threads.threadId);
      }
    });
    return server;
  };
